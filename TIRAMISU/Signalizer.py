...
def graphics_refresh():
    for c in range(-80, 80):
        for r in range(-48, 48):
            if 0 < Topographer.LANDMARK_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] <= 7:
            ...
            elif Topographer.WALL_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] == 1:
                ON_SCREEN_PALETTE[c + 80][r + 48] = COLOUR_LETHALWALL
            elif Topographer.LANDMARK_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] == 99:
                ON_SCREEN_PALETTE[c + 80][r + 48] = COLOUR_BLACKTILE
            elif Topographer.WALL_SPLASH_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] > 0:
                ON_SCREEN_PALETTE[c + 80][r + 48] = COLOUR_SPLASHWALL
            elif Topographer.PRESENCE_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] == 1:
                ON_SCREEN_PALETTE[c + 80][r + 48] = COLOUR_PRESENCE
            elif Topographer.WALL_MAP[round(Poser.ROBOT_POSITION_X + c)][round(Poser.ROBOT_POSITION_Y + r)] == 0:
